...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', function(e) {
      ...
      if (window.zappyContactFormLoaded) {
        (async function() {
          try {
            ...
            let message = '';
            if (age) message += `גיל: ${age}\n`;
            if (goals) message += `מטרות: ${goals}\n`;
            if (experience) message += `ניסיון: ${experience}\n`;
            ...
            const response = await fetch('https://qaapi.zappy5.com/api/email/contact-form', {
              ...
              body: JSON.stringify({
                websiteId: 'd2b8af12-2dfc-4ecb-a94d-38ae867e1c3f',
                name: fullName,
                email: email,
                subject: 'בקשה למפגש ניסיון - רן השרירן',
                message: message || 'בקשה למפגש ניסיון',
                phone: phone
              })
            });
      ...
      alert('תודה! קיבלנו את הבקשה שלך למפגש ניסיון. ניצור איתך קשר בהקדם.');
      contactForm.reset();
    });
  }
});
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "רן השרירן משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
